...
def show_quiz_interface():
    ...
    if prob["type"] == "mul":
        ...
        ans = st.text_input("답을 입력하세요", key=f"mul_ans_{st.session_state.q_idx}")
        if st.button("제출하기", key=f"mul_btn_{st.session_state.q_idx}"):
            _handle_mul(ans, prob, elapsed)
    else:
        st.markdown(f"## 🔢 나눗셈 문제: {prob['a']} ÷ {prob['b']} = ?")
        quo = st.text_input("몫", key=f"div_quo_{st.session_state.q_idx}")
        rem = st.text_input("나머지", key=f"div_rem_{st.session_state.q_idx}")
...
def show_result():
    ...
    total_correct = sum(1 for c in st.session_state.history if c)
...
def show_rank():
    ...
    if df.empty:
        ...
        totals = df.groupby(["이름","학교"])['점수'].sum().reset_index()
        totals = totals.sort_values(by='점수', ascending=False).reset_index(drop=True)
        totals['순위'] = totals.index + 1
        st.markdown("---")
        st.subheader("🥇 개인별 총점 Top 10")
        st.table(totals.head(10)[["순위","이름","학교","점수"]])

        school_totals = df.groupby("학교")['점수'].sum().reset_index()
        school_totals = school_totals.sort_values(by='점수', ascending=False).reset_index(drop=True)
        ...
        search_name = st.text_input("학생 이름을 입력하세요", key="student_name_input")
        if st.button("검색", key="search_btn") and search_name.strip():
            matched = totals[totals["이름"] == search_name]
            if matched.empty:
                st.warning(f"'{search_name}' 기록이 없습니다.")
            else:
                for _, row in matched.iterrows():
                    st.markdown(
                        f"**{row['이름']} ({row['학교']}) - 총점 {row['점수']}점 (순위 {row['순위']})**"
                    )
    if st.button("◀ 뒤로 가기"):
        st.session_state.show_rank = False
        reset_quiz_state()
