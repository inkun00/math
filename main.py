...
def get_gspread_client():
    ...
    # st.secrets["gcp_service_account"]에 JSON 전체가 dict로 들어 있어야 합니다.
    creds_dict = st.secrets["gcp_service_account"]
    creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
    client = gspread.authorize(creds)
    return client
    ...
    with col1:
        if st.button("시작하기"):
            if name.strip() == "":
                ...
                # 버튼 클릭 후 자동으로 rerun되어 show_quiz_interface() 호출
    with col2:
        if st.button("순위 보기"):
...
def show_quiz_interface():
    ...
    # **곱셈 문제 출력** (이 부분이 없으면 화면에 문제가 보이지 않습니다)
